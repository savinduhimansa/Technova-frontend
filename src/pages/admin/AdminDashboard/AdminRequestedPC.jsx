...
const AdminRequestedPC = () => {
  ...
  // Filtered requests based on filter state
  ...
  const downloadPdf = () => {
    ...
    autoTable(doc, {
      startY: 20,
      head: [["Customer Email", ...partsList.map((p) => p.toUpperCase()), "Total Price", "Status"]],
  ...
  const renderTable = (data) => (
    <div className="overflow-x-auto rounded-xl border border-[#BFDBFE] bg-[#DBEAFE] shadow-sm">
      <table className="w-full text-sm text-left">
        ...
        <tbody>
          {data.map((req) => (
            <tr key={req._id} className="border-b border-[#BFDBFE] hover:bg-[#EFF6FF] text-[#1E3A8A]">
