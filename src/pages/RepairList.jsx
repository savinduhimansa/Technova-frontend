...
export default function RepairList() {
  ...
  const fetchRepairs = () => {
    setLoading(true);
    axios
      .get(toUrl("/app/repair")) //  no token headers
      .then((res) => {
        setRows(Array.isArray(res.data) ? res.data : []);
        setErr(null);
      })
      .catch((e) => {
        console.error("GET /app/repair failed:", e.message);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setBusyId(id);
      await axios.delete(toUrl(`/app/repair/${id}`));
      fetchRepairs();
    } catch (e) {
      console.error("DELETE /app/repair/:id failed:", e.message);
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex flex-wrap gap-2 justify-center">
                      ...
                      <a
                        ...
                        href={toUrl(`/app/repair/${r._id}/pdf`)}
                        target="_blank"
                        rel="noreferrer"
                      >
                        PDF
