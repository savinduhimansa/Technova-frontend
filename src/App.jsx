      ...
      <header className="border-b border-slate-800/60 sticky top-0 backdrop-blur bg-slate-950/40">
        <div className="mx-auto max-w-7xl px-4 py-4 flex items-center justify-between">
          ...
          <nav className="flex gap-4">
            <Link className={`hover:text-sky-300 ${pathname.startsWith("/builder") ? "text-sky-300" : ""}`} to="/builder">Builder</Link>
            <Link className={`hover:text-sky-300 ${pathname.startsWith("/admin") ? "text-sky-300" : ""}`} to="/admin">Admin</Link>
          </nav>
        </div>
      </header>
